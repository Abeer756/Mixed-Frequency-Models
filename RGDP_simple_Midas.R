## ===========================
# Mixed-Frequency MIDAS (Nealmon) — COMPLETE SCRIPT FOR RGDP
# ===========================
...
# y_rq <- ts(rgdp_q$RGDP_oil, start = rq_start, frequency = 4)   # alt
# y_rq <- ts(rgdp_q$RGDP_Noil, start = rq_start, frequency = 4)   # alt
...
for (i in seq_along(spec_list)) {
  ...
  if (inherits(best, "try-error")) {
    cat("ERROR fitting model", i, ":", conditionMessage(best), "\n")
    next
  }
...
plot_midas_weights <- function(summary_table, results,
  ...
  # build weights using uniform kmax (simple version)
  w_block <- function(v) {
    nealmon(get2(top$best$fit, v), top$best$kmax)
  }
...
# call it:
cat("\n==== GENERATING MIDAS WEIGHTS PLOT ====\n")
plot_midas_weights(summary_table, results)
cat("-> RGDP MIDAS weights plot saved as: rgdp_midas_weights.png\n")