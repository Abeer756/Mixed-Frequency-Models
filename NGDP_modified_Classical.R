...
# ==============================================================================================================
# Variable Dictionary (base names without _g suffix)
# ==============================================================================================================
var_labels <- c(
...
# ==============================================================================================================
# Quarterly Aggregation and Fitting Function
# ==============================================================================================================
fit_quarterly_spec <- function(xnames, add_ar = TRUE, fun = mean, robust = TRUE) {
  ...
  # Standardized betas for predictor blocks (not AR terms)
  sdy <- sd(Zdf$y_ng)
  std_beta <- setNames(numeric(length(xq_names)), xq_names)
  for (p in xq_names) {
    std_beta[p] <- coef(fit)[p] * sd(Zdf[[p]]) / sdy
  }
  
  # Coefficient table with robust p-values
  if (robust) {
  ...
  ct_pred$var_label <- unname(var_labels[label_key(ct_pred$var)])
  ...
  lb <- tryCatch(
    Box.test(residuals(fit), lag = 8, type = "Ljung-Box")$p.value, 
...
keep_if_available <- function(v) {
  if (all(sapply(v, present))) v else NULL
}
...
std_table$var_label <- unname(var_labels[label_key(std_table$var)])

cat("\n===============================================\n")
cat("STANDARDIZED BETAS WITH P-VALUES\n")
cat("===============================================\n")
print(std_table[order(std_table$group, -abs(std_table$std_beta)), 
                c("model_id","group","var","var_label","std_beta","estimate","p.value","sig")],
      row.names = FALSE)
