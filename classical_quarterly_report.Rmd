...
suppressPackageStartupMessages({
  library(dplyr); library(readxl); library(lubridate); library(zoo); library(janitor)
  library(broom); library(lmtest); library(sandwich); library(kableExtra)
})
```
...
\[
y_t = \alpha + \boldsymbol{\beta}^{\top}\mathbf{X}^{(Q)}_t + \phi_1 y_{t-1} + \phi_2 y_{t-2} + \phi_4 y_{t-4} + \varepsilon_t,
\]

where \(\mathbf{X}^{(Q)}_t\) are **quarterly** regressors constructed by aggregating **monthly log-growth** of each predictor *within the quarter* using the mean. This “classical” alternative **does not use MIDAS lag polynomials**; it sacrifices within‑quarter timing for parsimony and transparency.
...
# Specifications (Classical, No MIDAS)

We evaluate the following bundles, mirroring your custom groups:

- **Oil-export bundles:** A\_oilex, B1\_oilex (add M1), B2\_oilex (add M2), C1\_oilex (add M1 + oil price), C2\_oilex (add M2 + oil price).
- **Production bundles:** A\_prod (replace oil exports with production), B1\_prod (add M1), B2\_prod (add M2), C1\_prod (add M1 + oil price), C2\_prod (add M2 + oil price).
...
Coefficients below are **conditional on AR controls** \(y_{t-1}, y_{t-2}, y_{t-4}\). For brevity, AR terms are not shown in the tables.
...
## Standardized Effect Sizes (β\_std)

```{r std-betas, results='asis'}
...
The **oil-export bundles** generally dominate by AIC/BIC. Inference based on HC1 robust errors consistently identifies **Oil & Gaz Exports** as a positive and statistically significant driver of real GDP growth across oil-export specifications (e.g., A\_oilex, B1\_oilex, C1\_oilex, C2\_oilex). Money aggregates and oil prices present mixed evidence: M2 is occasionally significant in production bundles (e.g., B2\_prod), and oil price enters negatively but only marginally significant in some oil-export models. AR dynamics (lags \(t-1, t-2, t-4\)) soak up persistence; coefficients reported above are **conditional** on these AR controls.

Out-of-sample, the best performers by information criteria also achieve competitive forecast errors (RMSE/MAE/MASE), with no residual autocorrelation or strong heteroskedasticity per Ljung–Box and Breusch–Pagan tests.

# Reproducibility Notes

- Changing the quarterly aggregator from `mean` to `sum` (for flow variables) or to the `last` month can be done by passing a different `fun` to `fit_quarterly_spec`.
- The classical block intentionally **excludes MIDAS lag polynomials**. A parallel MIDAS block can be added to this Rmd to compare timing‑rich distributed‑lag models versus the aggregated approach.
